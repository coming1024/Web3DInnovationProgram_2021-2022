...
ResourceLoader.prototype={
    init:function (url,camera,unitProcess) {
        ...
        loader.load(this.url+"resourceInfo.json", function(str){//dataTexture
            ...
            scope.resourceList=new ResourceList(resourceInfo,scope.camera);//获取资源列表//resourceInfo是什么？？
            scope.loadGeometry(camera);
            scope.loadMap();
        });
    },
            ...
            if(!fileName){//如果当前没有需要加载的几何文件
                ...
                scope.loader.load(scope.url+fileName, (gltf) => {
                   ...
                   // scope.unitProcess(gltf);
                    ...
                    //console.log(mesh0)
                    // if (mesh0.position.x-camera.position.x<20 &  mesh0.position.x-camera.position.x>-20){
                         scope.unitProcess(gltf);
                         scope.object.add(scene);
                    // }
...
ResourceList.prototype={
    ...
    getModelList:function(){//返回在视锥内且未被加载的资源列表//思考：如果在此处返回在范围内且未被加载的资源列表
